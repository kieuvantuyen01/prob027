...
\subsection{Phân tầng easy, medium và hard}

Kích thước mô hình tăng đồng thời theo $N$ và $c$. Generator dùng chỉ số
cấu trúc
\begin{equation}\label{eq:structural-score}
  B(N,c)=N^2(c-1).
\end{equation}
Đây chính là số biến đóng góp đơn vị $u_{ijh}$ dùng để đếm tổng click trong
mô hình SAT ở \cref{eq:unit-sum}. Generator sắp xếp tất cả các cặp $(N,c)$
theo $B$, sau đó chia danh sách thành ba phần bằng nhau. Với cấu hình mặc
định, sáu cặp thuộc mỗi nhóm như sau:

\begin{table}[H]
\centering
\caption{Phân nhóm theo kích thước cấu trúc $B=N^2(c-1)$.}
\label{tab:size-tiers}
\begin{tabularx}{\textwidth}{@{}l X l@{}}
\toprule
\textbf{Nhóm} & \textbf{Các cặp $(N,c)$ theo thứ tự tăng của $B$}
& \textbf{Khoảng $B$} \\
\midrule
Easy
& $(4,2),(5,2),(4,3),(6,2),(4,4),(5,3)$
& $16$--$50$ \\
Medium
& $(8,2),(6,3),(5,4),(10,2),(6,4),(8,3)$
& $64$--$128$ \\
Hard
& $(12,2),(8,4),(10,3),(12,3),(10,4),(12,4)$
& $144$--$432$ \\
\bottomrule
\end{tabularx}
\end{table}

\begin{notebox}[Cách hiểu đúng nhãn độ khó]
Easy, medium và hard ở đây là nhãn \emph{kích thước cấu trúc}, được xác định
trước khi chạy solver. Runtime thực tế còn phụ thuộc target, optimum và trạng
thái SAT/UNSAT. Sau thực nghiệm có thể bổ sung
\texttt{observed\_difficulty}, nhưng không thay đổi nhãn cấu trúc ban đầu.
\end{notebox}

Cấu hình benchmark đầy đủ là:
...
\begin{tabularx}{\textwidth}{@{}l X l@{}}
\toprule
\textbf{Thành phần} & \textbf{Cách lấy mẫu} & \textbf{Số instance} \\
\midrule
Witness cơ sở
& Một instance cho mỗi $(N,c,\rho)$, với
$N\in\{4,5,6,8,10,12\}$, $c\in\{2,3,4\}$ và
$\rho\in\{0.10,0.30,0.60\}$
& 54 \\
Witness bổ sung
& Hai instance thêm cho mỗi nhóm độ khó; mật độ được luân phiên
& 6 \\
Uniform-target
& Hai instance cho mỗi nhóm độ khó; giá trị $c$ được luân phiên
& 6 \\
\bottomrule
\end{tabularx}
\end{table}

Phần witness trước hết phủ toàn bộ các tổ hợp $(N,c,\rho)$:
\[
  6\times3\times3=54.
\]
Sáu witness bổ sung làm tổng witness bằng 60. Sáu uniform-target được chia
đều hai instance cho mỗi nhóm. Tổng kích thước benchmark là
\[
  60+6=66.
\]
Mỗi nhóm easy, medium và hard có 20 witness và 2 uniform-target, tức 22
instance. Cách phân bổ này giữ được độ phủ của witness nhưng tránh tạo hàng
trăm file gần giống nhau.
...
\[
  ...
  \operatorname{SHA256}(S,N,c,\text{họ},\rho,r,a)\right),
\]
...
\begin{enumerate}
  \item đọc một tầng $(N,c,\text{họ},\rho,r)$ và tính
        $B=N^2(c-1)$;
...
tại. Bộ 66 instance nằm trong \texttt{data/benchmark\_v1} và được sinh bằng:
...
\texttt{size\_tier/family/nNN\_cCC/}. File \texttt{manifest.csv} là chỉ mục dùng
...
{
  "schema_version": "2.0",
  "name": "easy_n04_c03_witness_d100_r000",
  "N": 4,
  "c": 3,
  "target": [[...], [...], [...], [...]],
  "generation": {
...
\texttt{dataset\_summary.json}. Ví dụ chạy toàn bộ nhóm easy:

\begin{lstlisting}[style=jsonstyle,language={}]
python3 models/sat_variant2.py \
  --input-dir data/benchmark_v1/easy
\end{lstlisting}
